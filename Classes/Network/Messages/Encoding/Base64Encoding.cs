using System;

namespace Trebuchet.Classes.Network.Messages.Encoding
{
    static class Base64Encoding
    {
        /// <summary>
        /// Int in, bytes of Base64 out.
        /// </summary>
        /// <param name="i"></param>
        /// <param name="numBytes"></param>
        /// <returns></returns>
        public static byte[] EncodeInt32(Int32 i, int numBytes)
        {
            byte[] bzRes = new byte[numBytes];

            for (int j = 1; j <= numBytes; j++)
            {
                int k = ((numBytes - j) * 6);

                bzRes[j - 1] = Convert.ToByte(0x40 + ((i >> k) & 0x3f));
            }

            return bzRes;
        }

        /// <summary>
        /// Bytes of Base64 in, int out.
        /// </summary>
        /// <param name="bzData"></param>
        /// <returns></returns>
        public static Int32 DecodeInt32(byte[] bzData)
        {
            int i = 0;
            int j = 0;

            for (int k = bzData.Length - 1; k >= 0; k--)
            {
                int x = bzData[k] - 0x40;

                if (j > 0)
                {
                    x *= Convert.ToInt32(Math.Pow(64.0, Convert.ToDouble(j)));
                }

                i += x;
                j++;
            }

            return i;
        }
    }
}
